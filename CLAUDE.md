...
- 3종: `creditorList`(원금+이자·세로 합계·안분율 100%) / `incomeExpense`(수입−생계비 = 가용소득, 표준생계비 초과 확인) / `rehabPlan`(가용소득×횟수 ≥ 청산가치, 변제기간 상한, 채권자별 안분).
...
- `users.sensitive_consent_at`에 동의 시각 기록(입증책임은 처리자에게 있다 — 제22조 제3항). 화면 체크만으로는 부족하다.
